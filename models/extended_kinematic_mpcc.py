...
class ExtendedKinematicModel:
    """
    states - [x, y, vx, yaw angle, vy, yaw rate, steering angle, theta]
    inputs - [drive force (proportional to acceleration), steering velocity, vi]
    ...
    def get_model_matrix(self, state, control_input):
        ...
        f_val = self.get_f(state, control_input)
        x_next = state + f_val * self.config.DTK
        C = x_next - A @ state - B @ control_input
        # print(A)
        # print(B)
        # print(C)
    ...
    def predict_motion(self, x0, control_input, dt):
        ...
        input_prediction = np.zeros((3, self.config.TK + 1))
